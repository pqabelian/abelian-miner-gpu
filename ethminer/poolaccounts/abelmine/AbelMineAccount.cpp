...
bool AbelMineAccount::storeRegisteredAccount(std::string poolHost, std::string workingDir)
{
    if (!workingDir.empty())
    {
        int ret = chdir(workingDir.c_str());
    ...
    // append username to account file
    std::string accountFile = poolHost + abelMineAccountFileExtension;
    std::ofstream ofs;
    ofs.open(accountFile,std::ios::app);
    if(!ofs.is_open()) {
    ...
    ofs << "username=" << m_user << std::endl;
    ofs.close();

    std::cout << "Username " << m_user << " has been written to account file " << accountFile << " successfully." << std::endl;
    std::cout << "In next run, please use the username in the abelminer command." << std::endl;
