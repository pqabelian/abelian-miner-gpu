...
uint8_t * SHA256::doubleHash256(const uint8_t * data, size_t length) {
    update(data, length);
    uint8_t * first = digest();
    SHA256 doubleSHA;
    doubleSHA.update(first, 32);
    return doubleSHA.digest();
}
...
std::string SHA256::toString(const uint8_t * digest) {
    ...
    for(uint8_t i = 32 ; i > 0 ; i--) {
        s << std::setw(2) << (unsigned int) digest[i-1];
    }
